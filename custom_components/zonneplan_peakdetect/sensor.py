...
class BatteryOptimizerSensor(SensorEntity, RestoreEntity):
    ...
    def _calculate_action_schedule(self, forecast_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        while current_idx < n - 1:
            # Slice of remaining prices
            remaining_prices = prices[current_idx:]

            # Find the index of the absolute minimum price in the remaining window (The Valley)
            local_min_val = min(remaining_prices)
            valley_offset = remaining_prices.index(local_min_val)
            valley_idx = current_idx + valley_offset
            

            # Find the index of the absolute maximum price AFTER that valley (The Peak)
            remaining_prices = remaining_prices[valley_idx:]
            if not remaining_prices:
                break
            local_max_val = max(remaining_prices)
            peak_offset = remaining_prices.index(local_max_val)
            peak_idx = valley_idx + peak_offset

            # Define the segment from current position to the peak
            segment = prepared_data[current_idx : peak_idx + 1]
            
            # Process if profit threshold is met
            if (local_max_val - local_min_val) >= self._min_profit_eur_kwh:
                interval_count += 1
